...
inline bool ExactSub(unsigned sup,vector<khash_t(32)*> Sub, khash_t(32)* G2Sid,vector<unsigned> ID2sup){
    ...
    while(!Q.empty()){
        ...
        for(unsigned i=G.h[u];i<G.h[u+1];++i){  
            ...
            if(ID2sup[ID]>= sup && !inqueue[w]) {Q.push(w);inqueue[w]=true;++sum_H;}
        }
    }
...
inline bool ResultSub(vector<khash_t(32)*> Sub, khash_t(32)* G2Sid, vector<unsigned> &ID2sup){
    ...
    for(unsigned i=G.h[query];i<G.h[query+1];++i){
        ...
        if(kh_get(32,G2Sid,u)==kh_end(G2Sid)) continue;

        unsigned ID = EdgeIDinSub(query,u,Sub,G2Sid);
        if(ID2sup[ID] > up_cur) up_cur = ID2sup[ID];
    }

    //computing the size of up_cur-truss subgraph in Sub

     up_cur = up_cur < ku-2?up_cur:ku-2;
    if( ExactSub(up_cur,Sub,G2Sid,ID2sup)){kl=up_cur+2; return true;} 
    // if(up_cur <= kl -2) return false;

    set<unsigned,greater<unsigned>> sup;
    for(unsigned val:ID2sup){
        if(val < up_cur)sup.insert(val);
    }

    //set<unsigned,greater<unsigned>>:: iterator it = sup.find(up_cur);
    for(set<unsigned,greater<unsigned>>:: iterator it = sup.begin();it!=sup.end();++it){
        unsigned sup = *it;
 
        if(ExactSub(sup,Sub,G2Sid,ID2sup)) {kl=sup+2; return true;}
    }
...
inline void RUpdate(unsigned val,unsigned short * State){
    ...
    // updata R due to the vertex is selected form R
    RRH.erase(val);     

   // updata R due to C grows
    State[val] = 1;
    for(unsigned i = G.h[val]; i<G.h[val+1];++i){
        unsigned v = G.e[i];
        if(State[v]!=1 && G.t[i]>=ku) {
            unordered_map<unsigned,unsigned>::iterator it = RRH.find(v);
            if(it == RRH.end()){RRH.insert(make_pair(v,1)); State[v] = 2;}
            else{++ it->second;}
        }
    }
...
inline void Raddupdate(unsigned val,unsigned short * State){
    ...
    for(unsigned i = G.h[val]; i<G.h[val+1];++i){
        unsigned v = G.e[i];
        if(State[v]==0 && G.t[i]>kl ) {
            map<unsigned,unsigned>:: iterator it = RR.find(v);
            if(it == RR.end()){RR.insert(make_pair(v,1));State[v] = 2;}
            //else{++ it->second;}
        }
    }
...
void ScoreSta(unsigned short *State, vector<khash_t(32)*> Sub, khash_t(32)* G2Sid, 
    ...
    while(!Q.empty()){
        ...
        for(unsigned i = G.h[u];i<G.h[u+1];++i){
            ...
            if(ID2sup[ID]>= kl-2){
                ...
                for(unsigned j = G.h[u];j<G.h[u+1];++j){   
                    ...
                    if(kh_get(32,khash[w],v)!= kh_end(khash[w])){    
                        ...
                        unsigned ID1 = EdgeIDinSub(u,v,Sub,G2Sid);

                        unsigned ID2 = EdgeIDinSub(v,w,Sub,G2Sid);

                        if(ID2sup[ID1] >= kl-2 && ID2sup[ID2] >= kl-2){           
                            ++ sum_tri;
                        }
            ...
            if(ID2sup[ID]>= kl-2 && !inqueue[w]) { Q.push(w);inqueue[w]=true;}
        }
...
inline unsigned ScoreExact(vector<unsigned>&VerBest, unsigned short * State){
    unsigned ver_score,ver_truss,ver_cur;
    unsigned best_ver = 0,best_score = 0,best_truss = 0;
    ...
    for(map<unsigned,unsigned>:: iterator it = RR.begin();it!=RR.end();++it){
        ...
        for(unsigned i = G.h[ver_cur]; i<G.h[ver_cur+1];++i){
            ...
            if(State[v]==1 && G.t[i]>kl ) {
                ++ ver_score;
                if(CC.find(v)->second < kl) ++sum_score;
            }
        }
 
        it->second = sum_score;
    
        if(ver_score==0) temp.emplace_back(ver_cur);  
...
inline void RaddVerBest(unsigned ver, unsigned short * State,unsigned kl_cur){
    ...
    //     }
    // }
    RR.insert(make_pair(ver,cur_sum));
    State[ver] = 2;
...
vector<unsigned> &ID2sup, vector<vector<unsigned>>& ID2end){
    ...
    for(map<unsigned,unsigned>::iterator it = CC.begin(); it != CC.end();++it){
        ...
        for(; it_cur != CC.end();++it_cur){
            ...
            if(flag){
                // updating budget
                if(bmax[id_ver]-Candidate[ID_Candidate][1] < Candidate[id_ver][1]) return false;
            }
        } 
...
vector<vector<unsigned>>& ID2end,bool flag,unsigned ver){
    ...
    if(flag){
        ...
        CC.insert(make_pair(ver,sum_deg));
...
void FrameWork(){
    ...
    if(flag == 0) {kl = ku;IsSuc = true; ScoFin();return;}        //[l,h]
